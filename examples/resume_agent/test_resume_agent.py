#!/usr/bin/env python3
"""
Simple test script for the Resume Agent functionality.

This script tests the core functionality without requiring LangGraph dependencies.
"""
...
def test_resume_models():
    ...
    try:
        from resume_models import ContactInfo, Resume, WorkExperience, JobDescription
...
def test_matching_tools():
    ...
    try:
        from matching_tools import extract_keywords, match_resume_with_jd, generate_resume_text
        from resume_models import ContactInfo, Resume, WorkExperience, JobDescription
...
def test_resume_agent_import():
    """Test that the resume agent can be imported."""
    print("\nTesting Resume Agent Import...")
    
    try:
        # Note: This will likely fail due to missing LangGraph dependencies
        # but we can at least test the import structure
        from resume_agent import ResumeAgent
        print("✓ Resume agent import works (dependencies available)")
        return True
        
    except ImportError as e:
        if "langgraph" in str(e).lower():
            print("⚠ Resume agent import failed due to missing LangGraph dependencies (expected)")
            print("  This is normal in a test environment without full dependencies")
            return True
        else:
            print(f"✗ Resume agent import failed: {e}")
            return False
    except Exception as e:
        print(f"✗ Resume agent import failed: {e}")
        traceback.print_exc()
...
def test_sample_workflow():
    ...
    try:
        from resume_models import ContactInfo, Resume, WorkExperience, Education, Project, JobDescription
...
def main():
    """Run all tests."""
    print("Resume Agent Test Suite")
    print("=" * 50)
    
    tests = [
        test_resume_models,
        test_matching_tools,
        test_resume_agent_import,
